...
-- Add proper data validation for critical fields
DO $$
BEGIN
  -- Add hourly rate constraint if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'check_hourly_rate_positive' 
    AND table_name = 'jobs'
  ) THEN
    ALTER TABLE public.jobs ADD CONSTRAINT check_hourly_rate_positive CHECK (hourly_rate > 0);
  END IF;
  
  -- Add movers constraint if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'check_movers_positive' 
    AND table_name = 'jobs'
  ) THEN
    ALTER TABLE public.jobs ADD CONSTRAINT check_movers_positive CHECK (movers_needed > 0);
  END IF;
  
  -- Add hourly wage constraint if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'check_hourly_wage_positive' 
    AND table_name = 'employees'
  ) THEN
    ALTER TABLE public.employees ADD CONSTRAINT check_hourly_wage_positive CHECK (hourly_wage > 0);
  END IF;
END $$;